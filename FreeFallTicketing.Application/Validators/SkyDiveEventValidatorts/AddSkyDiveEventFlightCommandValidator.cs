...
namespace SkyDiveTicketing.Application.Validators.SkyDiveEventValidatorts
{
    public class AddSkyDiveEventFlightCommandValidator : AbstractValidator<AddSkyDiveEventFlightCommand>
    {
        public AddSkyDiveEventFlightCommandValidator()
        {
            ...
            RuleFor(c => c.TickTypes.Count()).GreaterThan(0).WithMessage("حداقل یک نوع از بلیت را باید وارد کنید.");
            RuleFor(c=> c.TickTypes.Select(s=>s.TypeId).Count() - c.TickTypes.Select(s => s.TypeId).Distinct().Count()).Equal(0).WithMessage("نوع بلیت تکراری است.");
        }
    }
}
